...
- [ ] 整理启动参数、拓扑初始化和时间片更新输出。
- [ ] 区分 JsonTopo 状态、传统拓扑状态和聚类状态。
- [ ] 使用外部 `node_id` 打印节点和链路，避免暴露内部下标。
- [ ] 默认关闭非交付必需的调试文件输出。

## 4. Feeder Hold Time

- [ ] 仅对带正数 `hold_time` 的 `feeder` 链路启动计时。
- [ ] 新增、恢复或更新 hold time 时正确重新调度。
- [ ] 链路被时间片删除或覆盖时，使旧定时事件失效。
- [ ] 到期后断开链路并重新计算 OSPF 路由。
- [ ] 合并同一仿真时刻到期链路的日志输出。
- [ ] 更新字段说明和 snapshot/patch 示例。

## 5. 甲方输入示例

- [ ] 纳入 `customer-73sat-6gs` JsonTopo 示例。
- [ ] 纳入与 73 节点示例匹配的流量矩阵。
- [ ] 保留最小 snapshot 和 patch 示例。
- [ ] 确认示例数据不会被默认目录扫描误加载。

## 6. 验证与交付

- [ ] `./waf build` 编译通过。
- [ ] 上一版 JsonTopo OSPF 用例无回归。
- [ ] 73 节点甲方示例使用显式 input 路径运行通过。
- [ ] snapshot 和 patch 更新模式运行通过。
- [ ] hold time 到期、取消和重新建立场景运行通过。
- [ ] 工作区不包含构建产物、跟踪文件或仿真输出。
- [ ] 每项改动以独立、可回退的提交保存。
