...
成功标准是：程序不写死节点规模或文件数量，能够从目录中最早的快照创建全部节点和
...
2. 自动选择目录中时间最早的快照，并映射到仿真 `0s`；
3. 只选择绝对时间落在
   `[最早快照, 最早快照 + simulationDuration]` 内的快照；
4. 后续文件按相对最早快照的秒数调度，支持跨日和非连续时间；
...
- 仿真时长 `300s` 时，自动从 `00-00-00` 开始，仅加载至 `00-05-00` 的快照；
...
- [x] 严格解析完整日期时间文件名。
- [x] 支持跨日、缺失分钟和重复时间检测。
- [x] 自动选择最早快照，并按仿真时长裁剪文件窗口。
- [x] 仅保存路径和相对时间，到点后读取快照。
- 验证：时长 `300s` 时自动从 `00-00-00` 开始，得到 5 个运行期事件，
  时间为 60、120、180、240、300 秒。
- 依赖：Task 3。

### Task 5：接入初始化、CLI 和运行时更新（M）

- [x] 新增 `--linkOutputDir` 和 `--simulationDuration`。
- [x] 新模式从最早快照创建节点和初始链路。
- [x] 后续快照更新卫星簇和完整链路状态。
- [x] 未启用新模式时保持现有入口行为。
...
- `clusterId` 是卫星簇编号；
- 甲方只填写正数仿真时长，单位为秒；
- 程序自动从目录中最早快照开始，只读取仿真时长窗口内的快照；
- 非法 JSON 由数据生成端修复，程序不做文本级猜测修补。

## 12. 实施与验证结果

- 新格式样例：不指定起始时间、运行 `300s`，自动从 `2024-01-02_00-00-00`
  开始，识别 24 颗卫星、4 个地面站、52 条初始链路，并应用五个更新；
- 簇变化：`300s` 对应 `00-05-00`，卫星 602 从簇 0 进入簇 1；
- feeder 变化：同一时间点断开旧 feeder 4 条并安装新 feeder 2 条；
- 跨日：最早文件为 `23:59` 时，次日 `00:00/00:01` 正确映射到 `60/120s`；
