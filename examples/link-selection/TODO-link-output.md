...
状态：甲方字段语义和时间窗口需求已确认，按本文任务拆分实现中。
...
- [ ] 新增 `--linkOutputDir`、`--linkOutputStartTime` 和
  `--simulationDuration`。
- [ ] 新模式从首个快照创建节点和初始链路。
- [ ] 后续快照更新卫星簇和完整链路状态。
- [ ] 未启用新模式时保持现有入口行为。
...
- [ ] 使用 `simulationDuration` 控制应用和 ns-3 停止时间。
