...
成功标准是：程序不写死节点规模或文件数量，能够从最早快照创建全部节点和
初始链路，并按文件名中的实际时间差依次应用后续完整快照。
...
- `delay` 按毫秒解释。星间链路典型值为 `32.648`，若按原协议的微秒解释，
  不符合该星间距离量级。
- `hold_time` 按毫秒解释，且表示当前快照时刻起的剩余时长。相邻一分钟快照中，
  同一 feeder 的值恰好减少 `60000`。
...
2. 按完整日期时间排序，最早文件映射到仿真 `0s`；
3. 其余文件按相对秒数调度，支持跨日和非连续时间；
4. 内存只保存时间戳和文件路径，不预加载 1440 个完整快照；
5. 到达时间片时才解析对应文件，并作为完整快照应用；
6. 仿真时长至少覆盖最后一个快照再加 60 秒。
...
- `hold_time=370065` 最终调度为 `370.065s`；
...
- [ ] 修复 5 个文件缺失的分隔逗号。
- [ ] 将 JSON 文件权限规范为普通数据文件。
- [ ] 增加新格式 README，记录字段、单位和地面站推导规则。
- 验证：8 个文件逐一通过 `jq empty`。
- 依赖：无。

### Task 2：支持毫秒单位且保留旧字段（M）

- [ ] 让 feeder hold time 支持毫秒换算后的非整数秒。
- [ ] 新格式 `delay` 转换为微秒保存，保留三位毫秒精度。
- [ ] 旧格式的 `delay` 微秒和 `hold_time` 秒语义保持不变。
- 验证：构建通过，旧 73 星样例结果无回归。
- 依赖：Task 1。

### Task 3：解析混合数组并推导节点（M）

- [ ] 区分链路项和 `sat_id/clusterId` 项。
- [ ] 从最早快照建立卫星集合。
- [ ] 从 feeder 的非卫星端点建立地面站集合。
- [ ] 拒绝未知对象形状、重复卫星和非法 feeder。
...
- [ ] 支持跨日、缺失分钟和重复时间检测。
- [ ] 仅保存路径和相对时间，到点后读取快照。
- 验证：8 个文件得到 7 个运行期事件，时间为 60 至 420 秒。
- 依赖：Task 3。
...
- [ ] `offeredload=0` 时跳过流量矩阵读取和客户端数据分配。
- [ ] 移除当前与 `totalTimeStep` 成比例但实际未使用的业务矩阵维度。
- [ ] 新模式默认停止时间覆盖最后快照再加 60 秒。
- 验证：1440 个文件的调度清单可建立，内存不随快照内容总量线性增长。
- 依赖：Task 5。

### Task 7：文档、全量回归与交付清单（S）

- [ ] 更新主 README 和 JsonTopo 格式说明。
- [ ] 运行新旧两套端到端命令。
- [ ] 列出本次新增、修改和甲方需替换的文件。
- [ ] 确认所有任务以独立提交推送。
- 依赖：Task 6。
...
| 字段单位与推断不一致 | 时延和 hold time 错三个数量级 | 在 README 固化单位并用样例值验证 |
...
以下内容已依据样例推断，若甲方定义不同，应先更新本文再实现：

- `delay` 的单位为毫秒；
- `hold_time` 的单位为毫秒，含义为当前时刻起的剩余时长；
- `clusterId` 是卫星簇编号；
- 最后一个快照后默认继续运行 60 秒；
