 ...
 *
 * Authors:  Xueyu Lu (luxueyu@stu.xidian.edu.cn)
 *           Chengru Liu
 * 
 * Reference: global-route-manager-impl.h
 */

#ifndef SAT_GLOBAL_ROUTE_MANAGER_IMPL_H
#define SAT_GLOBAL_ROUTE_MANAGER_IMPL_H
...
const uint32_t SATSPF_INFINITY = 0xffffffff; //!< "infinite" distance between nodes

// 需与para.cc一致
const uint32_t ISLnum = 4;      // 卫星节点星间链路条数
const uint8_t Indexsat = 0;     // 卫星起始ID
const uint32_t NumOfOrbit = 6;
const uint32_t SatPerOrbit = 11;

class SatCandidateQueue;
class Ipv4GlobalRouting;

Ipv4Address GetIPAddress(Ptr<ns3::Node> node, uint32_t interface);
Ipv4Address GetIpFromInterface(Ptr<Node> node, uint32_t interface);
Ipv4Address GetNeiIpFromInterface(Ptr<Node> node, int32_t interface);
uint32_t GetIdFromIp(NodeContainer& nodes, Ipv4Address ip);
uint32_t GetIdFromIpSim(Ipv4Address ip);
Ptr<Node> GetNodeFromIP(Ipv4Address ip);
Ptr<Node> GetNodeFromId(uint32_t id);
Ipv4RoutingTableEntry* GetNextFromTable(Ptr<Node> ScrNode, Ptr<Node> DestNode);
Ipv4RoutingTableEntry* GetSatNextFromTable(Ptr<Node> ScrNode, Ptr<Node> DestNode);
std::pair<int32_t, int32_t> GetInterface(Ptr<Node> ScrNode, Ptr<Node> DesNode);
bool IsBoundaryNode(Ptr<Node> node, uint8_t NextClusterNum);
bool InRegionforWalkerStar (uint32_t curSatId, uint32_t dstSatId, uint32_t boundSatId, uint32_t numOfOrbit, uint32_t satPerOrbit);  // 判断边界卫星是否在矩形域内 (Walker Star)
bool InRegionforWalkerDelta (uint32_t curSatId, uint32_t dstSatId, uint32_t boundSatId, uint32_t numOfOrbit, uint32_t satPerOrbit); // 判断边界卫星是否在矩形域内 (Walker Delta)
int32_t GetBackupInterface(Ptr<Node> CurNode, Ptr<Node> DstNode, int32_t nxtIf, uint8_t index, bool consType, uint32_t numOfOrbit, uint32_t satPerOrbit); // 获取备份下一跳转发接口

extern uint32_t ROUSTEP;
 ...
 */
  SATSPFVertex(GlobalRoutingLSA* lsa);
 ...
 */
  void SetRootExitDirection (Ipv4Address nextHop, int32_t id = SATSPF_INFINITY);
 ...
 */
  void SetRootExitDirection (SATSPFVertex::NodeExit_t exit);
  /**
   ...
   */
  void MergeRootExitDirections (const SATSPFVertex* vertex);
  /**
   ...
   */
  void InheritAllRootExitDirections (const SATSPFVertex* vertex);
  /**
 ...
 * in the SPF tree.
 */
  SATSPFVertex* GetParent (uint32_t i = 0) const;
 ...
 */
  void SetParent (SATSPFVertex* parent);
  /**
   ...
   */
  void MergeParent (const SATSPFVertex* v);
 ...
 * the new child.
 */
  uint32_t AddChild (SATSPFVertex* child);
...
private:
  ...
  typedef std::list<SATSPFVertex*> ListOfSATSPFVertex_t; //!< container of SPFVertexes
  ListOfSATSPFVertex_t m_parents; //!< parent list
  ListOfSATSPFVertex_t m_children; //!< Children list
 ...
 */
  SATSPFVertex& operator= (SATSPFVertex& v);
   ...
   */
  friend std::ostream& operator<< (std::ostream& os, const SATSPFVertex::ListOfSATSPFVertex_t& vs);
};
 ...
 */
class SatGlobalRouteManagerLSDB
{
public:
/**
 ...
 */
  SatGlobalRouteManagerLSDB (SatGlobalRouteManagerLSDB& lsdb);
 ...
 * @returns the copied object
 */
  SatGlobalRouteManagerLSDB& operator= (SatGlobalRouteManagerLSDB& lsdb);
};
 ...
 */
class SatGlobalRouteManagerImpl
{
public:
  SatGlobalRouteManagerImpl ();
  virtual ~SatGlobalRouteManagerImpl ();
/**
 ...
 */
  ...
  virtual void DeleteClusteringGlobalRoutes (NodeContainer& nodes);

  /**
 * @brief Delete local static routes on single nodes that have a
 * GlobalRouterInterface
 *
 * \todo  separate manually assigned static routes from static routes that
 * the global routing code injects, and only delete the route which interface is equal to \param interface
 */
  virtual void DeleteLocalRoutes (Ptr<Node> node, uint32_t interface);

  virtual void DeleteRoutesForNodes (Ptr<Node> nodeSrc, Ptr<Node> nodeDst);
 ...
 */
  ...
  virtual void ClusteringBuildGlobalRoutingDatabase(NodeContainer& nodes, bool RouMode);
...
/**
 * @brief Compute Intra-Clustering and Inter-Clustering routes using a Dijkstra SPF computation and populate
 * per-node forwarding tables
 * @param RouMode 0-Intra Cluster Routing;   1-Inter Cluster Rouitng
 */
  virtual void ClusteringInitializeRoutes(NodeContainer& nodes, std::vector<std::vector<std::vector<std::pair<uint32_t, uint32_t>>>> BoundaryNode, bool RouMode, std::map<ns3::Ipv4Address, double> RlinkUtilization, std::map<ns3::Ipv4Address, std::vector<double>> clusterNetworkInf);

  virtual void ClusteringInitializeRoutesSim(NodeContainer& nodes);

/**
 * @brief 协同簇内簇间路由
 * @param cluster 簇内卫星节点
 * @param GlobalNode 全网卫星节点
 * @param AbstractClusterNodes 抽象拓扑节点
 * @param BoundaryNode 边界节点（本地簇号 邻居簇号 多个卫星ID对（ , ））
 * @param consType    星座类型
 * @param numOfOrbit  卫星轨道数
 * @param satPerOrbit  每个轨道卫星数
 */
  virtual void AddClusterRoutingTables(NodeContainer& cluster, 
 ...
 * @param satPerOrbit  每个轨道卫星数
 */
  virtual void ClusteringAddBackupRoutes(   NodeContainer& cluster,
                                            NodeContainer& GlobalNode,
                                            bool consType,
                                            uint32_t numOfOrbit, 
                                            uint32_t satPerOrbit);
 ...
 */
  void DebugUseLsdb (SatGlobalRouteManagerLSDB*);
 ...
 */
  SatGlobalRouteManagerImpl (SatGlobalRouteManagerImpl& srmi);
 ...
 * @returns the copied object
 */
  SatGlobalRouteManagerImpl& operator= (SatGlobalRouteManagerImpl& srmi);

  SATSPFVertex* m_spfroot; //!< the root node
  SatGlobalRouteManagerLSDB* m_lsdb; //!< the Link State DataBase (LSDB) of the Global Route Manager

  std::map<ns3::Ipv4Address, double> m_deviceU;         //!<linkUtilization for each link>
  std::map<ns3::Ipv4Address, double> m_clusterCost;

  double m_maxClusterDiameter;
  ...
  /**
   * \brief Calculate the shortest path first (SPF) tree  ----> intra Cluster Rouing
   *
  ...
  /**
   * \brief Calculate the shortest path first (SPF) tree for Clusters   ----> inter Cluster Rouing
   *
   * Equivalent to quagga ospf_spf_calculate
   * \param root the root node
   */
  void SPFCalculateforCluster (Ipv4Address root, std::vector<std::vector<std::vector<std::pair<uint32_t, uint32_t>>>> BoundaryNode);
   ...
   */
  void SPFProcessStubs (SATSPFVertex* v);
   ...
   */
  void ProcessASExternals (SATSPFVertex* v, GlobalRoutingLSA* extlsa);
   ...
   */
  void SPFNext (SATSPFVertex* v, SatCandidateQueue& candidate);

  void SPFNextforClusters (SATSPFVertex* v, SatCandidateQueue& candidate, std::vector<std::vector<std::vector<std::pair<uint32_t, uint32_t>>>> BoundaryNode);

  double ComputeClusterLBU(uint32_t curClusterId, uint32_t neiClusterId, std::vector<std::vector<std::vector<std::pair<uint32_t, uint32_t>>>> BoundaryNode);
  /**
   ...
   */
  int SPFNexthopCalculation (SATSPFVertex* v, SATSPFVertex* w, 
   ...
   * \param v the vertex
   */
  void SPFVertexAddParent (SATSPFVertex* v);
   ...
   */
  GlobalRoutingLinkRecord* SPFGetNextLink (SATSPFVertex* v, SATSPFVertex* w, 
   ...
   *
   */
  void SPFIntraAddRouter (SATSPFVertex* v);
   ...
   * \param v the vertex
   */
  void SPFIntraAddTransit (SATSPFVertex* v);
   ...
   * \param v the vertex
   */
  void SPFIntraAddStub (GlobalRoutingLinkRecord *l, SATSPFVertex* v);
   ...
   * \param v the vertex
   */
  void SPFAddASExternal (GlobalRoutingLSA *extlsa, SATSPFVertex *v);
...
#endif /* SAT_GLOBAL_ROUTE_MANAGER_IMPL_H */
