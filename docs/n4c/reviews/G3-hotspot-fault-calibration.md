...
beta=3 / run 11 / F3 off（启动 `28a5a83a6`，干净）：71 次 F1、2 次 F2，direct=67/0，
733 完成/67 失败；1715 条概率全部匹配。START 温度 min/median/max 为
20.7984/24.3905/27.6446°C，pF1 为1.4180/14.3183/46.6764%；恢复1.1687–3.2753 s，
中位2.2740 s；相邻 START 最短8 s、中位16 s，墙钟428.38 s。

## beta 冻结（正式联合实验前）

选择 **beta=3** 并设为默认：在已比较候选中提供更可观察的中温条件概率、更多直接中断，
START 温度/概率/恢复时长没有病态单点聚集，也符合用户倾向。67 仍比目标79低15.19%，
不把71次F1事件冒充67个direct，不宣称精确命中79。beta5/6只做纯曲线测试，未另跑
C800；不引入新的概率乘数、任务修改或额外候选来逼近配额。

从此冻结模型与输入。接下来 calibration=11/12/13、validation=21/22/23、run11 audit-off
重复均启用同一个 controlled F3。它们可独立并行运行；首次查看新模型验证组结果在本次
冻结提交之后，绝不据其结果回调参数。每个 run 都保留原始输出，不弃选。

beta=3 默认值重编译后，37 Python、15 C++、6 smoke、4 regression 再次全部通过；
100任务功能回归为82完成/18失败，460条概率匹配。F1-only/F2-only/联合小用例
分别175/136/90条全部匹配，默认关闭审计和重复运行清理合同保持通过。
