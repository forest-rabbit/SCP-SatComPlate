...
四组均从干净提交5af0d0cef启动，原正式w64 TaskTrace不变，F3关闭；墙钟492–501s。

| beta/gamma | F1 START | F1 direct | START温度P50 °C | pF1 P50/P90 % | busy P50 s | 22–25°C / 29.9–30°C START | 概率匹配行 |
|---|---:|---:|---:|---:|---:|---:|---:|
| 8/1.5 | 42 | 42 | 27.3969 | 12.4722/28.5578 | 17.3236 | 4/0 | 1753 |
| 8/2 | 53 | 51 | 27.8025 | 17.2105/33.5890 | 16.4188 | 6/0 | 1753 |
| 10/1.5 | 36 | 35 | 28.2549 | 17.4688/43.9352 | 20.9816 | 0/0 | 1777 |
| 10/2 | 43 | 43 | 28.1093 | 15.0933/32.7965 | 18.4092 | 1/0 | 1758 |

各轮F2均2次、direct=0；四种概率共7041行全部匹配，479个实际冷却点通过。
冻结 **beta=10、gamma=1.5**：四者中温START最少、pF1中位及P90最高，无临界堆积，
升温形状也较gamma2温和。不按direct数量选取；35最少亦不回调。
仍未完全达到建议的pF1中位20–30%/P90约50%，保留差距，不能写作所有形状目标达标。
该候选动态恢复2.560849–3.865590s、中位3.463057s；同节点START间隔最短16s、中位27s。

热点筛选起使用无预留F3节点的纯placement输入（f3=null），同一地理/哈希规则。
原四组pilot使用旧输入是为了固定比较条件，后续w64也须重跑，不把输入变化伪装成仅改权重。
大任务F3生成器只读none任务时序；同时过滤已到达但尚未开始的RESULT依赖。
已补充选择确定性、strict >200MB、WU进度、队列/未来RESULT、前缀不变及故障后排除测试，
当前39个Python测试通过。只有final F3的三轮标定均值可作为75–83验收依据。
