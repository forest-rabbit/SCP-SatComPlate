...
| 路径 | 当前引用/来源 | 分类 | 处理 |
|---|---|---|---|
| `src/**` | ns-3 上游模块 | `KEEP_UPSTREAM_UNUSED` | 全部保留；不得修改核心模块 |
| `src/openflow/**` | ns-3 上游可选模块，SatCompute 构建未启用 | `KEEP_UPSTREAM_UNUSED` | 保留；继续由 `--enable-modules=satcompute` 排除 |
| `src/core/helper/csv-reader.cc` | ns-3 core 通用 CSV reader；SatCompute legacy loader 未调用它 | `KEEP_UPSTREAM_UNUSED` | 原样保留 |
| `src/core/helper/csv-reader.h` | ns-3 core 通用 CSV reader；SatCompute legacy loader 未调用它 | `KEEP_UPSTREAM_UNUSED` | 原样保留 |
...
| `build-support/**` | ns-3 上游构建基础设施 | `KEEP_UPSTREAM_UNUSED` | 全部保留 |
| `test.py`、`waf`、根 `wscript` | ns-3 上游测试和构建入口 | `KEEP_UPSTREAM_UNUSED` | 全部保留 |
...
## 4. Legacy CSV/TCP offered-load 输入体系
...
KEEP_UPSTREAM_UNUSED:
  all ns-3 upstream source, examples, bindings, scratch and build infrastructure
