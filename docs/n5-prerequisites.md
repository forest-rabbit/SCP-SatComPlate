...
量、计量口径和适用边界，不是 N5 实现合同。后续任务映射和平台接入已由
[N4C最终合同](n4c/README.md)完成；下文压力实验描述的是当时的独立历史基线。
...
在该次实验时默认关闭；当前正式场景默认开启链路指标。完整输入、吞吐量、等待与运行成本见[10 Gbps 压力基线](pressure-10g-baseline.md)。
...
- S/W/K、sigma/H及四类任务已接入；当前映射见[生成模块](../contrib/satcompute/tools/generation/README.md)。
  LLM采用结构公式与合成计算量，不是实际推理/恢复标定；不能套用图像式rho。
