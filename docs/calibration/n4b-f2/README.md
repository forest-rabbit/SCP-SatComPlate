...
插值或空洞填补。色标数值轴保持线性，并等距标出 `0,1,...,13`；为使热点区域在视觉
上更连贯，计数 0--8 使用蓝色梯度的前 80%，计数 8--13 使用最后 20%。因此 8 和
13 都属于深色，但仍保留渐变差异。该分段只改变颜色梯度的取样位置，不移动刻度、
截断数值或修改原始整数计数。原始事件、整数网格和曝光归一化故障率继续保存为可
审计证据。
...
- 图宽 7.2 英寸，PNG 以 600 dpi 导出为 4115 x 1795 像素；SVG 保留可编辑文字，
...
- 图像没有地球底图、局部手工修补、网格平滑、插值或事件抽样。固定经纬度视窗仅
  用于放大展示，0--8/8--13 的分段蓝色梯度仅控制颜色映射；全部 1888 个事件位置
  仍以圆圈叠加并保留在源 CSV 中。
