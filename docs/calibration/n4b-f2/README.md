...
[PDF](n4b-f2-spatial-validation.pdf)。图中不使用地球底图，而以经度 `[-120,60]`、
纬度 `[-60,30]` 的固定视窗放大并近似居中显示故障域；SAA 矩形外的理论风险严格
为 0。该视窗裁剪只改变论文图的呈现范围，不平移、缩放或删除任何卫星坐标与故障
事件，完整原始坐标继续保存在证据 CSV 中。左图显示非对称理论风险场，右图显示
每个网格的 3 x 3 邻域平滑故障数并叠加全部 1888 个原始故障位置。右图色标固定
从 0 到未平滑网格的最大故障数 13，不再使用“每百万 exposure”的刻度。
...
- 图像没有地球底图、局部手工修补或事件抽样。唯一的数据变换是已公开权重的
  3 x 3 邻域网格平滑；固定经纬度视窗仅用于放大展示，全部 1888 个事件位置仍以
  圆圈叠加并保留在源 CSV 中。
