...
| [`protection/`](protection/README.md) | N5A-G2 无故障固定备份流、检查点与存储合同；恢复留给 G3 |
...
`protectionMode=off` 默认保持现有行为；N5A-G2 的 `fixed` 接入真实备份数据流，
仅允许无故障网络任务且 shadow 关闭。10 GB 备份池与固定 delta/n 的参数说明见
