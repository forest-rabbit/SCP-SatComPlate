...
CapacityAwareHrwPolicy::CapacityAwareHrwPolicy(const CapacityAwarePathView& pathView,
...
CapacityAwareHrwPolicy::FindBestSuffix(const PathSelectionContext& context,
                                       ...
                                       std::set<uint32_t>& visiting) const
{
    if (currentSatelliteId == context.destinationSatelliteId)
    {
        ...
        destination.bottleneckRateBps = std::numeric_limits<uint64_t>::max();
        return destination;
    }
    ...
    std::vector<EcmpRouteCandidate> candidates =
        m_pathView->GetEcmpRouteCandidates(currentSatelliteId, context.destinationSatelliteId);
    ...
    if (!candidates.empty())
    {
        ...
        {
            const EcmpRouteCandidate& candidate = candidates[rank.candidateIndex];
            uint32_t nextSatelliteId =
                m_pathView->GetNextHopSatelliteId(currentSatelliteId, candidate.outputInterface);
            CapacityAwarePathHop hop = {
                currentSatelliteId,
                nextSatelliteId,
                candidate,
                m_pathView->GetIslDataRateBps(currentSatelliteId, candidate.outputInterface)};
            uint64_t residualRate = m_reservationState->GetResidualRateBps(hop);
            if (residualRate == 0)
            {
                continue;
            }

            PathSearchResult suffix = FindBestSuffix(context, nextSatelliteId, memo, visiting);
            if (!suffix.found)
            {
                continue;
            }
            uint64_t pathRate = std::min(residualRate, suffix.bottleneckRateBps);
...
bool
CapacityAwareHrwPolicy::FindPath(const PathSelectionContext& context, CapacityAwarePath& path) const
{
    NS_ABORT_MSG_IF(context.sourceSatelliteId == context.destinationSatelliteId,
                    "capacity-aware path 要求不同源和目的卫星");
    std::map<uint32_t, PathSearchResult> memo;
    std::set<uint32_t> visiting;
    PathSearchResult result = FindBestSuffix(context, context.sourceSatelliteId, memo, visiting);
    if (!result.found)
    {
        path = CapacityAwarePath();
        return false;
    }
    NS_ABORT_MSG_IF(result.hops.empty() || result.bottleneckRateBps == 0 ||
                        result.bottleneckRateBps == std::numeric_limits<uint64_t>::max(),
