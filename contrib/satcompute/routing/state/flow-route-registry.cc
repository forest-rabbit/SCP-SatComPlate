...
{
    static TypeId typeId = TypeId("ns3::FlowRouteRegistry")
                               .SetParent<Object>()
...
bool
FlowRouteRegistry::NodeFlowKey::operator<(const NodeFlowKey& other) const
{
    return std::make_tuple(nodeId, flowKey) < std::make_tuple(other.nodeId, other.flowKey);
}
...
{
    NS_ABORT_MSG_IF(transferId == 0, "flow route registry 要求正 transfer_id");
    NS_ABORT_MSG_IF(declaredBytes == 0, "flow route registry 要求正 declared_bytes");
    FlowRouteMetadata metadata = {transferId, declaredBytes, false};
    NS_ABORT_MSG_IF(!m_flows.insert(std::make_pair(flowKey, metadata)).second,
                    "flow route registry 出现重复 five-tuple");
    NS_ABORT_MSG_IF(!m_flowKeysByTransferId.insert(std::make_pair(transferId, flowKey)).second,
...
FlowRouteRegistry::BeginSending(const EcmpFlowKey& flowKey)
{
    ...
    NS_ABORT_MSG_IF(flow == m_flows.end(), "flow route registry 无法启动未登记 flow");
    NS_ABORT_MSG_IF(flow->second.senderActive,
                    "flow route registry 重复启动 transfer_id=" << flow->second.transferId);
    ...
    NS_ABORT_MSG_IF(m_activeFlowCount == std::numeric_limits<uint32_t>::max(),
                                        ...
                                        FlowRouteFinalizationReason reason)
{
    ...
    {
        bool sameFlow =
            !(assignment->first.flowKey < flowKey) && !(flowKey < assignment->first.flowKey);
        if (!sameFlow)
        {
            ++assignment;
            continue;
        ...
        ReleaseAssignment(released, releaseAction, released->second.latestRouteEpoch);
    }
...
{
    auto flow = m_flows.find(flowKey);
    NS_ABORT_MSG_IF(flow == m_flows.end(), "flow route registry 不包含请求的 flow");
    return flow->second;
}

bool
FlowRouteRegistry::FindAssignment(uint32_t nodeId,
                                  const EcmpFlowKey& flowKey,
                                  FlowRouteAssignment& assignment) const
{
    NodeFlowKey key = {nodeId, flowKey};
...
void
FlowRouteRegistry::RecordAssignment(uint32_t nodeId,
                                    const EcmpFlowKey& flowKey,
                                    ...
                                    const std::string& selectionReason)
{
    ...
    NodeFlowKey assignmentKey = {nodeId, flowKey};
    FlowRouteAssignment assignment = {candidate, flow->second.declaredBytes, routeEpoch};
    NS_ABORT_MSG_IF(!m_assignments.insert(std::make_pair(assignmentKey, assignment)).second,
                    "flow route registry 的 node+flow assignment 已存在");

    SizeAwareLoadChange loadChange =
        m_sizeAwareLoadState.Reserve(nodeId, candidate, assignment.reservedBytes);
    RecordEvent("ASSIGN",
                selectionReason,
                routeEpoch,
                nodeId,
                                      ...
                                      const std::string& selectionReason)
{
    ...
    uint64_t candidateReserved = GetReservedBytes(nodeId, assignment->second.candidate);
...
void
FlowRouteRegistry::ReleaseInvalidAssignment(uint32_t nodeId,
                                            const EcmpFlowKey& flowKey,
                                            uint64_t routeEpoch)
{
    NodeFlowKey key = {nodeId, flowKey};
    ...
    ReleaseAssignment(assignment, "RELEASE_CANDIDATE_INVALID", routeEpoch);
}

void
FlowRouteRegistry::ReleaseAssignmentsForRouteUpdate(const EcmpFlowKey& flowKey, uint64_t routeEpoch)
{
    auto flow = m_flows.find(flowKey);
    NS_ABORT_MSG_IF(flow == m_flows.end() || !flow->second.senderActive,
    ...
    for (auto assignment = m_assignments.begin(); assignment != m_assignments.end();)
    {
        ...
        auto released = assignment++;
        ReleaseAssignment(released, "RELEASE_ROUTE_INVALIDATED", routeEpoch);
    }
}
...
uint64_t
FlowRouteRegistry::GetReservedBytes(uint32_t nodeId, const EcmpRouteCandidate& candidate) const
{
...
void
FlowRouteRegistry::RecordEvent(const std::string& action,
                               const std::string& selectionReason,
                               uint64_t routeEpoch,
                               uint32_t nodeId,
                               const EcmpFlowKey& flowKey,
                               ...
                               uint64_t totalReservedAfter)
{
    ...
    FlowRouteReservationEvent event = {Simulator::Now().GetNanoSeconds(),
                                       action,
                                       selectionReason,
                                       routeEpoch,
                                       nodeId,
