...
from ..topology.common.hash_utils import compact_json_bytes
...
def parse_compute_profile(
    ...
    *,
    ...
    for item in items:
        ...
        nodes.append(node)
    result = tuple(nodes)
    if result != tuple(sorted(result, key=lambda node: node.node_id)):
        raise ComputeProfileError("compute node IDs must be strictly increasing")
