...
左图使用 SAA 矩形四周各 10 度的动态视窗显示东西向非对称理论风险场；当前参数对应
经度 `[-100,15]`、纬度 `[-60,15]`。右图直接展示满足最低暴露要求的每个 2.5 度
方格的原始整数故障数，色标上限固定为原始网格最大故障数，并叠加全部实际故障位置。
右图不做邻域平滑、插值或空洞填补。色标使用线性的 `0,1,...,13` 等距整数刻度；
计数 0--8 使用蓝色梯度的前 80%，8--13 使用最后 20%，使高值方格颜色接近但仍可
区分。视窗与分段颜色梯度都只改变呈现，不移动数值刻度，也不修改事件 CSV、原始
整数网格或曝光归一化故障率。
