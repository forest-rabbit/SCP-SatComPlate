...
左图使用经度 `[-120,60]`、纬度 `[-60,30]` 的固定视窗显示东西向非对称理论风险
场；右图使用 3 x 3 邻域均值展示每个 2.5 度方格的故障数，色标上限固定为原始网格
最大故障数，并叠加全部未平滑的实际故障位置。视窗只放大论文图的有效区域，不修改
原始经纬度；平滑也只用于呈现，不覆盖事件 CSV、原始整数网格或曝光归一化故障率。
