...
def load_edge_trace(
    ...
    *,
    ...
    """Load complete edge sets from one unified scenario."""
    root = Path(scenario_dir)
    if validate:
        check_scenario(root)
    manifest = read_json(root / "scenario-manifest.json")
    if not isinstance(manifest, dict):
        raise EdgeStateError("scenario manifest must be an object")
    schedule = manifest["scenario_config"]["topology"]["schedule"]
    if "step_s" not in schedule:
        raise EdgeStateError("edge-state analysis requires dynamic topology")
    duration_s = schedule["duration_s"]
    step_s = schedule["step_s"]
    times = tuple(range(0, duration_s + 1, step_s))
    trace = load_topology_edge_trace(root / "topology")
    if trace.times != times:
        raise EdgeStateError(
            "scenario topology times differ from its dynamic schedule"
        )
    return trace


def load_topology_edge_trace(topology_dir: Path) -> EdgeTrace:
    """Load paired snapshots directly from a canonical topology directory."""
    root = Path(topology_dir)
    if not root.is_dir():
        raise EdgeStateError(f"topology directory does not exist: {root}")
    nodes_by_time = {}
    topology_by_time = {}
    for path in root.iterdir():
        ...
        if time_s in target:
            raise EdgeStateError("topology contains duplicate snapshot times")
        target[time_s] = path
    if (
        not nodes_by_time
        or set(nodes_by_time) != set(topology_by_time)
        or 0 not in nodes_by_time
    ):
        raise EdgeStateError("topology snapshots are not complete pairs")
    times = tuple(sorted(nodes_by_time))
    ...
    for time_s in times:
        ...
        topology_payload = read_json(topology_by_time[time_s])
        if (
        ...
        try:
            ...
            raise EdgeStateError(
                f"topology_{time_s}s.json link fields are invalid"
            ) from error
        links = parse_topology_payload(canonical_payload, node_ids)
        snapshots.append(
            (
                time_s,
                frozenset(
                    (link.node1_id, link.node2_id) for link in links
                ),
            )
        )
    if initial_node_ids is None:
