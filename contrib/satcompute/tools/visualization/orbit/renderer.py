...
class OrbitRenderer:
    ...
    def __init__(
        ...
        self.axes.set_title("SatCompute topology-trace visualization")
    ...
    def update(self, time_s: float) -> tuple[Any, ...]:
        ...
        source_time = self.scenario.source_snapshot_time_s(time_s)
        self.time_artist.set_text(
            f"simulation time: {time_s:g} s\n"
            f"source trace slice: {source_time:g} s"
        )
