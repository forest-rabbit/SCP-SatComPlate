...
本目录中的冻结 TaskTrace 以 10 Gbps 为参考，任务 120 到达时刻仍为
`1024.682825747 s`。带宽对照不得覆盖该文件；实验入口通过
`bandwidth_normalized_task_trace()` 生成独立派生输入，仅补偿任务 120 的名义 INPUT
序列化时差，使受控 F3 victim 在不同带宽下保持相同的参考计算相位。5 Gbps 对应
`1024.042825747 s`；若另行授权 2 Gbps 诊断，则对应 `1022.122825747 s`。
同一带宽内所有方案必须使用同一派生输入，旧实验继续引用原冻结文件。
