...
test_output="$(mktemp -d /tmp/satcompute-config-test.XXXXXX)"
...
./ns3 run --no-build \
  "satcompute-config-test --fixedScenario=$fixed --distanceScenario=$distance --taskScenario=$task --invalidScenario=$invalid --outputDir=$test_output"
