...
#include "ns3/compfrr-placement-policy.h"
#include "ns3/peak-quota-ledger.h"
...
template <class F> void Reject(F f)
{
    ...
    throw std::runtime_error("invalid CompFRR-P input accepted");
}
CompFrrForecast Forecast(uint64_t task = 1, uint32_t primary = 1)
{
    CompFrrForecast f;
...
CompFrrCandidate Candidate(uint32_t remote = 10)
{
    CompFrrCandidate c;
...
{
    auto f = Forecast();
    Near(CompFrrCatchSeconds(f), .3325, "V4 state catch-up equation");
    Near(CompFrrBudgetSeconds(f, S), 11, "original compute deadline budget");
    f.input.inputPolicy = InputStagingPolicy::DEFERRED;
    Near(CompFrrCatchSeconds(f), 1.3325, "deferred complete INPUT wait");
    f.inputLocal = true;
    Near(CompFrrCatchSeconds(f), .3325, "local delivery has exactly zero INPUT wait");
    f.config.batchN = 1;
    Near(CompFrrCatchSeconds(f), .25, "n=1 has neither tail nor remote merge term");
    ...
    for (uint32_t d = 10; d <= 100; ++d)
        for (uint32_t n = 1; n <= 1000 / d; ++n)
        {
            ...
            Near(CompFrrCatchSeconds(f), expected, "all fixed legal configurations honor V4");
        }
    f.config = {0, 1}; Reject([&] { CompFrrCatchSeconds(f); });
}
void Conflict()
{
    auto c = Candidate();
    auto s = ScoreCompFrrCandidate(c, CompFrrPlacementVariant::FULL);
    ...
    s = ScoreCompFrrCandidate(c, CompFrrPlacementVariant::FULL);
    ...
    auto windows = CompFrrRecoveryWindows(c.demand);
    ...
    Check(CompFrrRecoveryWindows(c.demand).empty(), "missing INPUT dependency cannot be ready");
    ...
    Reject([&] { ScoreCompFrrCandidate(c, CompFrrPlacementVariant::FULL); });
    c = Candidate(); c.peers = {Forecast(2, 2)};
    const auto end = CompFrrRecoveryWindows(c.peers.front()).front().endNs;
    c.demand.input.work = 3000; c.demand.input.deadlineNs = 50 * S;
    c.demand.input.risk.futureSteps = {{end, .5}};
    Near(ScoreCompFrrCandidate(c, CompFrrPlacementVariant::FULL).recoveryConflict, 0, "occupancy end is exclusive");
    c.demand.input.risk.futureSteps = {{end - 1, .5}};
    Near(ScoreCompFrrCandidate(c, CompFrrPlacementVariant::FULL).recoveryConflict, .5, "occupancy includes last ns");
    c.demand.input.risk.futureSteps = {{2 * S, 1.1}};
    Reject([&] { CompFrrRecoveryWindows(c.demand); });
}
void Ranking()
{
    CompFrrPlacementPolicy policy;
    ...
    Check(!ScoreCompFrrCandidate(a, CompFrrPlacementVariant::NO_M).feasible, "noM retains hard storage capacity");
    a = Candidate(); a.exposureNs = a.normalBusyNs = 0;
    Check(ScoreCompFrrCandidate(a, CompFrrPlacementVariant::FULL).historyUnavailable, "cold start has diagnostic");
    a.recoveryBusyNs = 1;
    Reject([&] { ScoreCompFrrCandidate(a, CompFrrPlacementVariant::FULL); });
    a = Candidate(); a.demand.input.deadlineNs = 2 * S;
    Check(!ScoreCompFrrCandidate(a, CompFrrPlacementVariant::FULL).feasible, "original deadline is a hard constraint");
    a = Candidate(); a.normalBusyNs = 8 * S; a.additionalQuotaBytes = 900;
    Near(ScoreCompFrrCandidate(a, CompFrrPlacementVariant::FULL).bottleneck, .9, "storage is dominant");
    Near(ScoreCompFrrCandidate(a, CompFrrPlacementVariant::NO_M).bottleneck, .8, "noM removes only scoring dimension");
    a = Candidate(); a.normalBusyNs = 8 * S;
    Near(ScoreCompFrrCandidate(a, CompFrrPlacementVariant::NO_U).bottleneck, .1, "noU preserves storage score");
    a.peers = {Forecast(2, 2)};
    Near(ScoreCompFrrCandidate(a, CompFrrPlacementVariant::NO_R).recoveryConflict, .5, "noR still records raw conflict");
}
void Quotas()
{
    PeakQuotaLedger ledger;
    ...
    Check(CompFrrPlacementPolicy(ComputePressurePolicy::CUMULATIVE).SelectRemote({Candidate()}).remoteNode ==
              CompFrrPlacementPolicy().SelectRemote({Candidate()}).remoteNode,
...
void RationalHistory()
{
    ...
    Near(IdleAwareComputePressure(.6, 10, 0), .6, "I=0");
    Near(IdleAwareComputePressure(.6, 10, 10), .3, "I=H");
    Near(IdleAwareComputePressure(.6, 10, 90), .06, "idle decay");
    Near(IdleAwareComputePressure(0, 10, 90), 0, "never busy pressure");
    Near(IdleAwareComputePressure(.6, INT64_MAX, INT64_MAX), .3, "integer horizon sum overflow");
    Reject([] { IdleAwareComputePressure(.6, 0, 0); });
    Reject([] { IdleAwareComputePressure(.6, 1, -1); });
    Reject([] { IdleAwareComputePressure(1.1, 1, 0); });
    double previous = 1;
    for (int64_t i = 0; i < 1000; ++i)
    {
        const double p = IdleAwareComputePressure(.6, 10, i);
    ...
    Check(CompFrrPlacementPolicy().SelectRemote({stale, fresh}).remoteNode == 11, "FULL redefined");
    auto policy = CompFrrPlacementPolicy(CompFrrPlacementVariant::RATIONAL_U);
    Check(policy.SelectRemote({stale, fresh}).remoteNode == 10, "rational ignored stale history");
    auto score = ScoreCompFrrCandidate(stale, CompFrrPlacementVariant::RATIONAL_U);
    ...
    Near(ScoreCompFrrCandidate(stale, CompFrrPlacementVariant::RATIONAL_U).bottleneck, .5, "rational removed R");
    stale.additionalQuotaBytes = 1001;
    Check(!ScoreCompFrrCandidate(stale, CompFrrPlacementVariant::RATIONAL_U).feasible, "rational bypassed storage");
    ...
    Check(ScoreCompFrrCandidate(stale, CompFrrPlacementVariant::RATIONAL_U).historyUnavailable,
...
int main()
{
    ...
    {
        std::cerr << "CompFRR-P V4 FAILED: " << e.what() << '\n';
        return 1;
    }
}
