...
### 2026-08-31：F2 对称空间 SEU 风险场（历史基线）

F2 风险在这一版首次改由卫星位置决定：SAA 内以 `(-60 deg,-28 deg)` 为热点中心，使用
`sigma_lon=18 deg`、`sigma_lat=12 deg` 的二维高斯平滑场；`theta_F2=0.5` 只控制
NOTICE，连续暴露和累计穿越概率只保留为统计量。`rho_SF=0.5` 将模型化 SEU 强度
映射为可恢复计算故障强度，F1/F2 仍使用两个独立随机流。

66 星空间加权扫描选择 `5210--6210s` 窗口，`A_F2^w=1337.3027`；以 1000 秒平均
2 次为目标，冻结 `lambda_SEU_max=0.0029910954712268908/s` 和
`kappa_F2=0.0014955477356134454/s`。100 个固定 run 的真实平台均值为 2.24，近似
95% 均值区间 `[1.9683,2.5117]` 包含目标 2。其后生成的 50 万秒空间图与 CSV/JSON
已随模型修订转入历史归档，不再用于当前验收。

- 空间模型实现：`9f4c4e965`
- 空间参数与标定：`193cb7b0f`、`3a9a2db54`
- 历史归档：`docs/calibration/n4b-f2/evidence/historical-symmetric/`

该对称经度基线随后被 2026-09-01 的非对称风险场取代。旧 CSV、JSON 和 PNG 仅归档
在 `docs/calibration/n4b-f2/evidence/historical-symmetric/`，不能作为当前模型的验收
结果。

### 2026-09-01：F2 东西向非对称风险场阶段一

F2 空间权重改为 two-piece Gaussian：热点西侧使用 `sigma_west=12 deg`，东侧使用
`sigma_east=24 deg`，纬度继续使用 `sigma_lat=12 deg`，从而得到西短东长的风险域。
`theta_F2=0.5` 和 `rho_SF=0.5` 保持不变；F1/F2 仍分别抽取随机数，`q_comp` 只对外
提供联合计算故障概率。

66 星重新扫描后冻结 `302--1302s` 窗口和 `A_F2^w=1398.9946`，以 1000 秒平均 2 次
为目标得到 `lambda_SEU_max=0.002859196111093899/s`、
`kappa_F2=0.0014295980555469494/s`。相同强度下，351 星与 720 星分别选择偏移
`4704s` 和 `3478s`，解析期望为 10.2225 与 20.6772 次。66 星 100 个固定 run 的
真实平台均值为 1.91，近似 95% 均值区间 `[1.6561,2.1639]` 包含目标 2。

本阶段只冻结模型、参数、星座窗口和 Monte Carlo 标定。66 星 100 万秒空间抽样、
论文候选图、联合预测/回放回归以及 PR/CI 均属于后续阶段，尚未在这里声明完成。

- 非对称空间模型：`9c82895a6`
- 非对称参数与窗口：`7d4a014e7`
- 当前标定说明：[N4B F2 空间辐射风险标定](docs/calibration/n4b-f2/README.md)

### 2026-09-01：F2 东西向非对称风险场阶段二

66 星使用阶段一冻结的参数和 `orbitStartOffset=302s` 完成 100 万秒 orbit-only
抽样，共执行 6600 万次原生位置查询。实际发生 1888 次 F2 故障，考虑 8 秒恢复抑制
后的条件期望为 1880.59，计数标准分数为 0.1709；风险—故障率相关系数为 0.8224。
高风险区只占 17.92% 的可抽样 SAA 暴露，却包含 52.07% 的故障。事件数、计数偏差、
风险集中性、高风险区富集和正相关五项门槛全部通过。

论文候选图使用 SAA 矩形四周各 10 度的固定边距和冻结蓝色梯度；当前视窗为经度
`[-100,15]`、纬度 `[-60,15]`。在不修改原始坐标的前提下，左图显示西短东长的
理论风险场，右图显示未平滑的原始网格故障数并叠加全部原始事件。右图使用线性的
`0,1,...,13` 等距整数刻度；计数 0--8 使用蓝色梯度前 80%，8--13 使用最后 20%，
使高值颜色接近但仍可区分。原始事件、整数网格与验收 JSON 和 PNG/SVG/PDF 一并
保存。正常平台运行仍默认关闭这些分析输出。

本阶段只完成 F2 长时空间验证和图像证据；联合预测、generate/replay 与既有 N4B
场景的完整回归属于阶段三，尚未在这里声明完成。

- 论文绘图实现：`ee897a8d4`、`05208d29e`
- 百万秒验证默认值：`ab2c6264d`
- 空间证据：[N4B F2 空间辐射风险标定](docs/calibration/n4b-f2/README.md)
