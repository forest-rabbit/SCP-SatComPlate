...
经纬度全域论文图、联合预测/回放回归以及 PR/CI 均属于后续阶段，尚未在这里声明
完成。
...
论文候选图使用经度 `[-120,60]`、纬度 `[-60,30]` 的固定视窗和冻结蓝色梯度，
在不修改原始坐标的前提下放大并近似居中展示故障域：左图显示西短东长的理论风险
场，右图显示 3 x 3 邻域平滑的网格故障数并叠加全部原始事件。右图色标固定为 0 到
原始峰值 13；原始事件、未平滑网格与验收 JSON 和 PNG/SVG/PDF 一并保存。正常平台
运行仍默认关闭这些分析输出。
